...
| Lyric Aligner | For non-musicians: one row per lyric line, matched to the score's phrases at the singer's breaths (rests), with that line's notes drawn as blocks and the syllables under them; pin a line start when the matcher guesses wrong. Click a note → Split (room for one more syllable), Merge with next, Longer, Shorter, Up/Down (one scale step, stays in key), Silence; click a rest → pickup note. Play just one line as a simple synth, or the original recording for that line's bars. Edits are saved per name and flow out of `abc` |
