...
| Lyric Aligner | For non-musicians: each section's sung notes drawn as blocks with your syllables under them. Click a note → Split (room for one more syllable), Merge with next, Longer, Shorter, Up/Down (one scale step, stays in key), Silence; click a rest → pickup note. Play the phrase as synth or the original recording. Edits are saved per name and flow out of `abc` |
